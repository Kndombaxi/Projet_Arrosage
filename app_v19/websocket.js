...
const setData = async (action, mode) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const token = await getToken();
            if (!token) {
                reject({ success: false, error: "Token d'authentification non disponible" });
                return;
            }
...
// Fonction pour récupérer les données historiques depuis la base de données
...
export {
    closeWebSocket, fetchHistoricalData, getData, getLatestModeState, getLatestSensorData, setData, setModeStateCallback,
    setRawDataCallback
};
