...
export default function DataScreen() {
  ...
  const processSensorData = (data: any): SensorData => {
    ...
    if (data && data.données) {
      ...
      if (deviceName === "Capteur d'humidité") {
        ...
        newSensorData.capteur1 = parseFloat(valeur as string);
      } else if (deviceName === "Capteur de présence d'eau" || deviceName === "Capteu de présence d'eau 2") {
        newSensorData.capteur2 = parseFloat(valeur as string);
      }
    ...
    if (data && data.dernieresDonnees && Array.isArray(data.dernieresDonnees)) {
      ...
      data.dernieresDonnees.forEach((item: any) => {
        ...
        if (deviceName === "Capteur d'humidité") {
          ...
          newSensorData.capteur1 = parseFloat(valeur);
        } else if (deviceName === "Capteur de présence d'eau" || deviceName === "Capteu de présence d'eau 2") {
          newSensorData.capteur2 = parseFloat(valeur);
        }
      });
  ...
  const processHistoricalData = (data: any): HistoricalData => {
    ...
    // Vérifier si les données sont au format InfluxDB
    if (Array.isArray(data)) {
      data.forEach(item => {
        try {
          // Détecter si les données sont au format InfluxDB
          if (item && item._time && item._value !== undefined && item._field) {
            // Utiliser la date directement si elle est au format chaîne
            ...
            // Classer les données selon le _field ou le tagname
            ...
            } else if ((item._field === "capteur1" || item.tagname === "Capteur eau supérieur à 500L" || item.tagname?.includes("500L"))) {
              processed.capteur1.push({
                date: formattedDate,
                valeur: item._value
              });
            } else if ((item._field === "capteur2" || item.tagname === "Capteur de présence d'eau" || item.tagname === "Capteu de présence d'eau 2")) {
              processed.capteur2.push({
                date: formattedDate,
                valeur: item._value
              });
            }
          ...
          // Format ancien pour la compatibilité
          else if (item && item.date && item.deviceName && item.valeur !== undefined) {
            let formattedDate = "00:00";
            ...
            } else if (item.deviceName === "Capteur eau supérieur à 500L" || item.deviceName.includes("500L")) {
              processed.capteur1.push({
                date: formattedDate,
                valeur: parseFloat(item.valeur)
              });
      ...
      processed.capteur1.sort((a, b) => a.date.localeCompare(b.date));
      processed.capteur2.sort((a, b) => a.date.localeCompare(b.date));
    }
  ...
  const fetchHistorical = async () => {
    ...
    try {
      ...
      console.log("Données historiques traitées:", processedData);
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.historyContainer}>
        ...
        {/* Graphique Capteur 1 (Eau supérieur à 500L) */}
        {historicalData.capteur1.length > 0 ? (
          <View style={styles.chartContainer}>
            <Text style={styles.chartTitle}>Capteur eau supérieur à 500L</Text>
            <LineChart
              data={{
                labels: formatDateLabels(historicalData.capteur1.map(item => item.date), selectedUnit),
                datasets: [{
                  data: historicalData.capteur1.map(item => item.valeur),
                  color: () => '#9c27b0', // Violet
                  strokeWidth: 2
                }]
              }}
          ...
          <View style={styles.noDataContainer}>
            <Text style={styles.noDataText}>Aucune donnée du capteur eau supérieur à 500L disponible</Text>
          </View>
        )}
        
        {/* Graphique Capteur 2 (Présence d'eau) */}
        {historicalData.capteur2.length > 0 ? (
          <View style={styles.chartContainer}>
            <Text style={styles.chartTitle}>Capteur de présence d'eau</Text>
            <LineChart
              data={{
                labels: formatDateLabels(historicalData.capteur2.map(item => item.date), selectedUnit),
                datasets: [{
                  data: historicalData.capteur2.map(item => item.valeur),
                  color: () => '#e91e63', // Rose
                  strokeWidth: 2
                }]
              }}
              ...
              chartConfig={{
                ...chartConfig,
                color: (opacity = 1) => `rgba(233, 30, 99, ${opacity})`, // Rose
              }}
              bezier
              style={styles.chart}
              yAxisLabel=""
          ...
          <View style={styles.noDataContainer}>
            <Text style={styles.noDataText}>Aucune donnée du capteur de présence d'eau disponible</Text>
          </View>
        )}
      </View>
    </ScrollView>
