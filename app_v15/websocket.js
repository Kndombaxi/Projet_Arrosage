...
const fetchHistoricalData = async (duree, unite) => {
    try {
        ...
        // Transformation des données InfluxDB en format compatible
        const transformedData = dataArray.map(item => {
            // Déterminer le type de capteur en fonction de _field
            let deviceName;
            if (item._field === "Humidité" || item.tagname === "Capteur sol") {
                ...
                deviceName = item.tagname || item._field;
            }
            
            // Formater la date en format ISO
            const date = new Date(item._time).toISOString();
