...
export default function HomeScreen() {
  ...
  const toggleWatering = async () => {
    ...
    try {
      ...
      // Désactiver l'indicateur de chargement après un délai pour permettre 
      // le traitement des messages WebSocket
      setTimeout(() => {
        setLoading(false);
      }, 500);
    }
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      const response = await fetch('https://api.arrosage.cielnewton.fr/modifierMDP', {
        method: 'POST',
        headers: {
      ...
      try {
        ...
        // Vérifier spécifiquement le message "Ancien mot de passe incorrect"
        if (data && data.message === "Ancien mot de passe incorrect") {
